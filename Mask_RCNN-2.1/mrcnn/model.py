...
# keras 2.10+: Layer moved from keras.engine to keras.layers
if not hasattr(KE, 'Layer'):
    KE.Layer = KL.Layer
...
#     bg_ids = np.where((rpn_roi_iou_max >= 0.1) & (rpn_roi_iou_max < 0.5))[0]
    ...
    for i in pos_ids:
        ...
        mask = skimage.transform.resize(
            m.astype(float), config.MASK_SHAPE,
...
class MaskRCNN():
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        exclude: list of layer names to exclude (forces by_name=True)
        """
        if exclude:
            by_name = True

        keras_model = self.keras_model
        inner = keras_model.inner_model if hasattr(keras_model, "inner_model") else keras_model

        if exclude:
            # Load weights by name, skipping excluded layers manually
            import h5py
            from keras.saving.hdf5_format import load_weights_from_hdf5_group_by_name
            f = h5py.File(filepath, mode='r')
            if 'layer_names' not in f.attrs and 'model_weights' in f:
                f = f['model_weights']
            filtered_layers = [l for l in inner.layers if l.name not in exclude]
            # Temporarily swap layers on a dummy object
            class _M:
                def __init__(self, layers):
                    self.layers = layers
            load_weights_from_hdf5_group_by_name(f, _M(filtered_layers))
            f.close()
        else:
            inner.load_weights(filepath, by_name=by_name)
