...
# Import Mask RCNN
sys.path.append(os.path.dirname(os.path.abspath(__file__)))  # To find local version of the library (mrcnn package next to this file)
...
class FacadeDataset(utils.Dataset):
    ...
    def load_facade(self, dataset_dir, subset):
        ...
        # Add images
        gt_dir='/home/wyf/Mask_RCNN-2.2/datasets/facade_gt/'
...
def detect_and_color_splash(model, image_path=None, savepath=None, video_path=None):
    import joblib
    ...
    if image_path:
        ...
        # Color splash
        joblib.dump((r['rois'],r['class_ids']),savepath+'/'+imagename+'.pkl')
        #with open(imagename+'.txt','w') as f:
         #   f.write(str(r['rois'])+str(r['class_ids'])+str(r['scores']))
        splash = color_splash(image, r['masks'])
        # Save output
        file_name = savepath+'/'+imagename+"_splash.png"
        splash_rgb = np.repeat(splash, 3, axis=2)
        skimage.io.imsave(file_name, splash_rgb)
    elif video_path:
        import cv2
        # Video capture
...
if __name__ == '__main__':
    ...
    if args.command == "train":
        ...
        for file in files:
            ...
            imagename = path_read + '/' + file
    
            image = skimage.io.imread(imagename)
            #l,m,n = image.shape
            #if l>50 and m>50:             
            detect_and_color_splash(model, imagename, path_save,
                    video_path=args.video)
            ...
            # path = os.getcwd()
            #NOTE we also generate boundingbox and probability
            file_name = file.split('.')[0]+'_'+str(ct)+"_bbox.png"
            path2 = path_save + file_name  
            plt.savefig(path2)
            ct+=1
