...
def adj_floor(H, hfset, hspset, label):
    ...
    hspmin = np.min(hspm) if hspm else range_para['h_gap'][0]
...
def adddoor(W, H, pwindows_set, s):
    ...
    if len(p_Stairwell_window)>1: #如果存在梯间窗户，则每个梯间窗的一楼为门的位置
        ...
        # A single facade photo/piece has at most one real entrance -- matching
        # the "no stairwell window detected" branch below, which always produces
        # exactly one door. xdoor's size depends on how many distinct horizontal
        # positions got swept up into p_Stairwell_window (a row x column
        # cross-product upstream in getrow_col()), which can be large even for a
        # single misclassified "stairwell" floor if that floor has many windows
        # across its width. Cap here, at the actual point doors get synthesized,
        # so the fix holds regardless of how that upstream classification
        # behaves. Note a single WallSurface can be a composite of many separate
        # facade pieces, each processed independently, so the wall-level door
        # count can still be a small multiple of 1, not exactly 1.
        MAX_DOORS_PER_FACADE = 1
        xdoor = xdoor[:MAX_DOORS_PER_FACADE]
