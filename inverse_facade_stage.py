...
def getrow_col(p_window,H,W,s=1,side='main'):
    ...
    if row!=[] and col!=[]:
        ...
        if slectrow > range_para['w_sp'][0]*s:
            ...
            start = 1
        i = len(sorted_pwindownew) - 1   # ensure i is bound if the loop body never runs (single-window facade)
        ...
        # A real building has very few actual stairwell/entrance cores (typically
        # 1-2), so cap how many distinct floor-levels this loop may classify as
        # "stairwell window". Without this, a building with several tightly-spaced
        # (but otherwise perfectly normal) floors can have most/all of them
        # misclassified as stairwell windows -- each one then gets a door
        # synthesized for it downstream in amendwindow22_stage_PATCH.adddoor(),
        # producing dozens of implausible doors on a single facade.
        MAX_STAIRWELL_COLUMNS = 2
        for i in range(1, len(coldisclean)):
            if len(col_Stairwell_window) >= MAX_STAIRWELL_COLUMNS:
                break
        ...
        if len(rowdisclean) * len(coldisclean) > 2000:
            print('skip: degenerate clustering produced', len(rowdisclean), 'x', len(coldisclean), 'candidates, treating facade as unreliable')
            return [], []
...
def check_and_remove_overlapping(rectangles): #删除重叠的矩形,对不满足矩形之间不相交、距离太近等约束条件的窗户矩形进行删除，维持生成结果的拓扑正确。
    n = len(rectangles)
    if n > 2000:
        print('skip: check_and_remove_overlapping got', n, 'rectangles, too many for the O(n^2) pairwise check, skipping dedup')
        return rectangles
    to_remove = set()
