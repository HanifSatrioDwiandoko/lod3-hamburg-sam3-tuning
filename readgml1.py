...
def polygon_area_3d(points):
    "Area of a planar (or near-planar) 3D polygon, via Newell's method -- works regardless of the wall's orientation."
    n = len(points)
    if n < 3:
        return 0.0
    nx = ny = nz = 0.0
...
def wall_span_length(points):
    """
    Distance between the same 2 'extreme' corner points that coor_opening4.get_rotation()
    picks (via s = sqrt(x^2+y^2)+|z|) to use as the wall's rotation axis. A well-formed
    facade wall's true area is close to span_length * height; a malformed/degenerate ring
    (e.g. one where most vertices cluster near a single corner with only a brief spike
    reaching elsewhere) can have a large bounding box and area while these two particular
    'extreme' points end up only centimetres apart -- which is what actually breaks
    get_rotation()'s placement, regardless of how large the polygon's area/height look.
    """
    s = [np.sqrt(x*x + y*y) + abs(z) for x, y, z in points]
    i1, i2 = int(np.argmin(s)), int(np.argmax(s))
    x1, y1, _ = points[i1]
    x2, y2, _ = points[i2]
