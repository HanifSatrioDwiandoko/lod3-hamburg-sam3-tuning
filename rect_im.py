...
def order_points(modelpts,psts):
    if modelpts is None or psts is None:
        return None, 0, 0
    # require at least four model points and four image points
    if len(modelpts) < 4 or len(psts) < 4:
        return None, 0, 0
    # ensure the two lists correspond in length; otherwise abort
    if len(modelpts) != len(psts):
        return None, 0, 0
    ...
    s = newmp.sum(axis = 1)
    psts_arr = np.asarray(psts)
    try:
        rect[3] = psts_arr[np.argmin(s)]
        rect[1] = psts_arr[np.argmax(s)]
    except Exception:
        return None, 0, 0
    ...
    try:
        rect[2] = psts_arr[np.argmin(diff)]
        rect[0] = psts_arr[np.argmax(diff)]
    except Exception:
        return None, 0, 0
...
def four_point_transform(image, mpts, psts):
    ...
    # individually
    if image is None or mpts is None or psts is None:
        return [], 0
    ...
    # order
        ...
        # pixels-per-metre. Deriving this from maxHeight loses accuracy because
        # maxHeight was int()-truncated above: downstream, W_metres = W_px/scale,
        # so the truncation inflates the wall's apparent width by up to
        # 1/maxHeight. On short, wide walls the rectified image is only a few
        # pixels tall, which inflated W by as much as 14% (e.g. 45.48m reported
        # for a wall that is really 39.80m wide) and pushed the window layout
        # off the end of the wall. maxWidth*hw is the untruncated pixel height,
        # so this is the same ratio without the rounding loss -- equivalently
        # maxWidth/w, making W_metres come out exactly equal to the wall width.
        scale = (maxWidth * hw) / h
    else:
        warped = []
        scale = 0
